...
suffix='lstm_ver4_2'
suffix1='ver4_2'
no_sp_per_vid=100 #no of splices per video
...
for i in folder:
	...
	if(fin_feat1==[]):
		...
		temp=np.load(temp[0]) #temp is a list of size = 1
		...
		temp=np.load(temp[0])
		np.random.shuffle(temp)
		fin_feat1=np.append(fin_feat1,temp[0:no_sp_per_vid,...],0) #0 refers to the axis of append



no_auto=np.shape(fin_feat1)[2]/feat_sz #2000/100
